...
public class BudgetPieChart extends AbstractDemoChart {
   ...
   */
  public Form execute() {
      ArrayList<Double> count = (ArrayList<Double>) serviceEnfant.getInstance().stat().get("ageEnfant");
      Double[] ds =count .toArray(new Double[count .size()]);
    //  System.out.println(count);
        double[] d = ArrayUtils.toPrimitive(ds);
     double[] values =d; //new double[] { 12, 14, 11, 10, 19 };
      System.out.println("d" +d.toString());
    int[] colors = new int[] { ColorUtil.WHITE, ColorUtil.GREEN, ColorUtil.MAGENTA, ColorUtil.WHITE ,ColorUtil.MAGENTA, ColorUtil.YELLOW,ColorUtil.MAGENTA};
